...
function drawHUD() {
  ...
  if (hint) {
    ...
    // sur tactile : en bas à gauche des boutons ronds (colonne droite d'environ 110 px), jamais au milieu de l'écran
    const touch = document.body.classList.contains('touch');
    const maxW = touch ? W - 150 - SA.l - SA.r : W - 40 - SA.l - SA.r, lines = [];
    const cx = touch ? SA.l + 12 + maxW / 2 : W / 2;
    let cur = '';
    ...
    const hy = H - 22 - SA.b;
    ctx.fillStyle = 'rgba(0,0,0,0.6)'; ctx.fillRect(cx - tw / 2, hy - 8 - 18 * lines.length, tw, 12 + 18 * lines.length);
    if (!hint.t0) hint.t0 = performance.now();
    let budget = Math.floor((performance.now() - hint.t0) / 16);
    ctx.fillStyle = '#ffd97a'; lines.forEach((l, i) => { const part = l.slice(0, Math.max(0, budget)); budget -= l.length + 1; ctx.textAlign = 'left'; const lw = ctx.measureText(l).width; ctx.fillText(part, cx - lw / 2, hy - 18 * (lines.length - 1 - i)); }); ctx.textAlign = 'center'; ctx.globalAlpha = 1;
  }
  // joysticks
