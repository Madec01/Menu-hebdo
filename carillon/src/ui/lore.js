...
// écran empilable 'leaf' : enter({ leafId }). À la première lecture, le Battant commente certains
// Feuillets (ui/battant.js, clé optionnelle battant.leaf_<id>) : toast et ligne sous le texte.
...
import { leafComment } from './battant.js';
...
/** Marque lu ; renvoie true à la première lecture. */
function markRead(id) {
  const s = getSave();
  if (!s.leavesRead) s.leavesRead = [];
  if (s.leavesRead.indexOf(id) >= 0) return false;
  s.leavesRead.push(id); commit();
  return true;
}
...
export function createLeafReader() {
  let id = 'f01', scroll = 0, lines = null, maxScroll = 0, comment = '';
  ...
  return {
    freezes: true,
    enter(p) {
      id = p.leafId; scroll = 0; lines = null;
      const first = markRead(id);
      comment = leafComment(id);
      if (first && comment) toast({ title: t('ui.toast.battant'), body: comment, icon: 'ui_coeur' });
    ...
    render(ui) {
      ...
      if (!lines) {
        lines = wrap(ui, t('lore.' + id + '.text'), TW, 'ui', 10).map((l) => ({ text: l, battant: false }));
        // Commentaire du Battant sous le texte, en retrait (police plus petite).
        if (comment) { lines.push({ text: '', battant: true }); for (const l of wrap(ui, comment, TW - 16, 'ui', 9)) lines.push({ text: l, battant: true }); }
        maxScroll = Math.max(0, lines.length * LH - (PH - TOP - 28));
      }
      ui.save(); ui.beginPath(); ui.rect(PX + 4, PY + TOP, PW - 8, PH - TOP - 26); ui.clip();
      for (let i = 0; i < lines.length; i++) {
        const l = lines[i];
        if (l.battant) text(ui, l.text, TX + 16, PY + TOP + i * LH - scroll, { size: 9, color: C.encreClaire });
        else text(ui, l.text, TX, PY + TOP + i * LH - scroll, { size: 10, color: C.encre });
      }
      ui.restore();
