...
const W = 480, H = 270;
const CARD_W = 96, GAP = 24, CARD_Y = 72, FLIP_SEC = 0.35, STAGGER = 0.22;
...
export function createLevelUp(deps) {
  ...
  const rerollRect = { x: W / 2 - 50, y: H - 40, w: 100, h: 18 };
  ...
  return {
    ...
    render(ui) {
      dimmer(ui, W, H, 0.6);
      heading(ui, t('ui.levelup.title', { level }), W / 2, 26, 22);
      text(ui, t('ui.levelup.subtitle'), W / 2, 54, { size: 10, align: 'center', color: C.os, shadow: true });
      for (let i = 0; i < 3; i++) {
      ...
      text(ui, t('ui.levelup.hint'), W / 2, H - 16, { size: 8, align: 'center', color: C.gris });
    },
  };
}
