...
export function judge(inputAt, subdivision = 1) { const sub = subdivision > 0 ? subdivision : 1; const b = bf(inputAt); const n = Math.round(b / sub) * sub; const offsetMs = (b - n) * st.beatDur * 1000; const a = Math.abs(offsetMs); return { grade: a <= st.windowMs / 3 ? 'parfait' : a <= st.windowMs ? 'bon' : 'rate', offsetMs, beat: n, early: offsetMs < 0, offbeat: n !== Math.round(n) }; }
