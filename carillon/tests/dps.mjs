#!/usr/bin/env node
// tests/dps.mjs — DPS théorique de chaque Timbre (niveaux 1 et 7), de chaque fusion (9) et de chaque
// Accord de dégâts (Contrepoids, Corde de chanvre, Étain, Écho : niveaux 0 → 5 sur chaque Timbre 7), sous Node.
...
// Règle d'équilibrage (audit § 2.2) : aucun Accord ne dépasse +25 % de DPS par niveau (colonne « max/niv »,
// tolérance ±3 points pour le bruit du tirage ; les paliers discrets — Étain niveau 3 — sont signalés).
// Accords comportementaux hors DPS (estimations, non mesurées ici) : Souffle = traînée de Volée 4 × niv dégâts
// par 0,5 s sur 3 disques de 14 px pendant 1 temps (≈ 8 × niv dégâts par Volée qui traverse) ; Ferrure = recul
// renvoyé 52 × niv px/s au contact ; Cire = régénération ×2 en série de Parfaits (+0,5 × niv PV/s) ;
// Métronome = fenêtre +10 %/niv (Résonance et cloche horaire).
//   node tests/dps.mjs            # tableaux
//   node tests/dps.mjs --json     # JSON
//   node tests/dps.mjs --data DIR # autre jeu de JSON
//   node tests/dps.mjs --no-accords   # sans la matrice des Accords (plus rapide)
...
const DT = 1 / 60;
const SECONDS = 30;
const DPS_ACCORDS = ['contrepoids', 'corde_de_chanvre', 'etain', 'echo'];
const MAX_PER_LEVEL = 0.25;
...
export async function measureAll(dataDir, { accords = true } = {}) {
  ...
  /** DPS d'une arme. `accord` : { id, level } ajoute cet Accord seul (matrice des Accords). */
  function measure(weaponId, level, scenario, fusion, extra = null, accord = null) {
    ...
    if (accord) for (let l = 0; l < accord.level; l++) passives.addPassive(p, accord.id);
    const ts = [];
    ...
    p.facing.x = 1; p.facing.y = 0; p.iframesT = 1e9;
    if (!accord || accord.id !== 'etain') p.stats.crit = 0;   // la matrice mesure Étain avec sa chance de critique
    weapons.resetReport();
    input.__setAxis(0, 0);
  ...
  // Matrice des Accords de dégâts : chaque Timbre 7 × chaque Accord, niveaux 0 → 5, scénario groupe (et seul pour la
  // cadence / le critique). Colonne « max/niv » = plus fort gain marginal d'un niveau au suivant.
  const matrix = [];
  if (accords) {
    for (const def of data.allWeapons().values()) {
      for (const a of DPS_ACCORDS) {
        const levels = [];
        for (let l = 0; l <= 5; l++) {
          const scen = a === 'contrepoids' ? 'groupe' : 'seul';
          levels.push(Math.max(measure(def.id, 7, scen, false, null, { id: a, level: l }), measure(def.id, 7, 'groupe', false, null, { id: a, level: l })));
        }
        let worst = 0, worstAt = 0;
        for (let l = 1; l <= 5; l++) { const r = levels[l - 1] > 0 ? levels[l] / levels[l - 1] - 1 : 0; if (r > worst) { worst = r; worstAt = l; } }
        matrix.push({ weapon: def.id, accord: a, levels, maxPerLevel: worst, at: worstAt, total: levels[0] > 0 ? levels[5] / levels[0] - 1 : 0 });
      }
    }
  }
  return { rows, matrix };
...
export function tableText(rows, matrix = []) {
  const lines = ['arme                          seul niv1  seul niv7  groupe niv1  groupe niv7'];
  for (const r of rows) lines.push(`${r.id.padEnd(28)} ${f(r.seul1).padStart(10)} ${f(r.seul7).padStart(10)} ${f(r.groupe1).padStart(12)} ${f(r.groupe7).padStart(12)}`);
  if (matrix.length) {
    lines.push('', `Accords de dégâts sur chaque Timbre 7 (DPS niveaux 0 → 5 ; règle : ≤ ${pct(MAX_PER_LEVEL)} par niveau)`);
    lines.push('arme               accord             niv0  niv1  niv2  niv3  niv4  niv5    total   max/niv');
    for (const m of matrix) {
      const flag = m.maxPerLevel > MAX_PER_LEVEL + 0.03 ? '  <-- dépasse' : '';
      lines.push(`${m.weapon.padEnd(18)} ${m.accord.padEnd(18)} ${m.levels.map((v) => f(v).padStart(5)).join(' ')}  ${pct(m.total).padStart(7)}  ${pct(m.maxPerLevel).padStart(7)} (niv ${m.at})${flag}`);
    }
  }
  return lines.join('\n');
}
...
if (isMain) {
  ...
  const { rows, matrix } = await measureAll(dir, { accords: !process.argv.includes('--no-accords') });
  console.log(process.argv.includes('--json') ? JSON.stringify({ rows, matrix }) : tableText(rows, matrix));
}
